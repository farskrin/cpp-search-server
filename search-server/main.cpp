...
public:
    ...
    void AddDocument(int document_id, const string& document) {
        ...
        {
            double tf = (double)count(words.begin(), words.end(), w)/words.size();
            word_to_document_freqs_[w].insert({ document_id, tf });
        } 
        document_count_++;
    }
...
private:    
    ...
    vector<Document> FindAllDocuments(const Query& query_words) const {
        ...
        for (const string& plus : query_words.plus) {
            ...
            {
                double idf = log((double)document_count_ / word_to_document_freqs_.at(plus).size());

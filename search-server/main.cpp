...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (document_id < 0) {
            throw invalid_argument("document_id < 0");
        }
        if (documents_.count(document_id)) {
            throw invalid_argument("Document with document_id already exist");
        }
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        ...
        if (text[0] == '-') {
            if (text.size() > 1) {
                if (text[1] == '-') {
                    throw invalid_argument("Minus word contain double hyphen");
                }
				is_minus = true;
				text = text.substr(1);
            } else {
                throw invalid_argument("Minus word contain only hyphen");
            }
        }
